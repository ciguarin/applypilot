...
def _review_summary(profile: dict) -> str:
    ...
    lines = [
        ...
        f"  [bold]2. Personal  [/bold] {p.get('full_name', 'not set')}  {p.get('email', '')}",
        f"  [bold]3. Education [/bold] {edu.get('degree', 'not set')}, {edu.get('institution', 'not set')}, GPA {edu.get('gpa', 'not set')}",
        f"  [bold]4. Profile   [/bold] {exp.get('target_role', 'not set')}, {len(all_skills)} skills",
