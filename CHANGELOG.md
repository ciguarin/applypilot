...
### Fixed
- **`_parse_negarprh`'s header-row detection used a substring match (`"company" in cells[0].lower()`) that false-triggered on a data row for "The Boeing Company"** (its own company-name cell contains the substring "company"), corrupting `headers` into garbage field names derived from Boeing's row values for the rest of the file. From that point on, `row.get("apply", "")` always returned an empty string, `_extract_last_url("")` returned `None`, and every subsequent row was silently dropped. Found while comparing the bespoke parser's job count (32) against the new generic parser's (65-66) on the same README and tracing the gap with an instrumented copy of the parsing loop. Out of ~1,364 real data rows in the current README, roughly 1,200 (88%) were being silently dropped -- meaning the project's single best discovery source (documented ~80% high-fit rate) has been running at a fraction of its true yield since this parser was first written. `_parse_hanzili` had the identical pattern (`"title" in cells[0].lower()`) and got the same fix pre-emptively; it wasn't confirmed to be actively triggering (no company name in that README currently contains "title" as a substring), but the bug class is identical and silent. Both now require an exact header-cell match. Verified against live README content: negarprh job count went from 32 to 65 after the fix; hanzili was unaffected (55 before and after), consistent with the bug not currently being live there.
- **A genuinely successful Workday application could be marked `failed:unverified_applied` and then auto-retried against a job it had already submitted**, risking a real duplicate application. `CONFIRMATION_PHRASE_RE` (the independent-evidence check required alongside the model's own `RESULT:APPLIED` marker, added specifically to catch the model just asserting success with no evidence) only recognized five literal phrases, and two real confirmations from the same overnight run -- Generac's "Congratulations! Thank you for your application" and Aquatic Informatics/Veralto's "Congratulations!" -- didn't match any of them, despite both transcripts showing an actual confirmation page and, for Generac, a `Job_Application_ID` in the resulting URL. Both got downgraded to `unverified_applied`, which wasn't in `PERMANENT_FAILURES`, so both were picked back up and re-submitted 2-3 more times before finally failing on a since-changed page state. Found while investigating a report of confirmation emails received for applications the DB didn't show as applied. Added "congratulations" and "thank you for your application" to the recognized-phrase list (the quote-mark requirement itself is unchanged, so an unquoted claim still doesn't count), and moved `unverified_applied` into `PERMANENT_FAILURES` so an ambiguous result is left in the manual queue for a human to check instead of being retried blind -- retrying something we can't confirm either already worked (duplicate risk) or didn't (wasted attempt), and only the operator can tell which. The two jobs above were manually corrected to `applied` in the DB (`applypilot apply --mark-applied <url>`) based on their transcripts' confirmation evidence.
